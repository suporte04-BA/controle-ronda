...
async function fetchGestorAdmins(admin: any): Promise<{ email: string; nome: string }[]> {
  ...
  const { data: setores } = await admin.from("setores").select("id,nome").ilike("nome", "%gestor%");
  if (setores?.length) {
    const setorIds = setores.map((s: any) => s.id);
    const { data: profs } = await admin.from("profiles").select("id,nome,email").in("setor_id", setorIds);
    if (profs?.length) {
